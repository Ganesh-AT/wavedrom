...
const archer = (res, Events, lane, customStyle, cfgScratchpad, index) => (element, edgeidx) => {
    ...
    if (fChar === '[') {
        ...
        const TDCSRegExp = new RegExp(/\[([^\]]*)\]([^[]*)\[([^\]]*)\](?:(?=\[)\[([^\]]*)\])?(?:(?=\[)\[([^\]]*)\])?\s*(.*|$)/);
        // -----------------------------[ TDCSpec ] shape  [ TDCSpec ]          [  arcCls  ]?       [  arrowClass ]? label?      --
        ...
        [1, 3].map( idx => { // Remove spaces only from TDCSpec components
            if (Match[idx] !== undefined) {
                Match[idx] = Match[idx].replace(/\s/g, '');
            }
        } );
