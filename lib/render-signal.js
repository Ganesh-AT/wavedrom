...
const fixWaveSkinForCustomSignals = require('./fix-waveskin-for-custom-signals.js');
...
function renderSignal (index, source, waveSkin) {
    ...
    fixWaveSkinForCustomSignals (source, waveSkin, ret);
    
    // affirmSourceConfig would have executed earlier and tracked all the siglane IDs.
    ret.lanes.map(function(siglane, currIdx) {
        let inLaneIdx = siglane.overlayOnLane ;
        let inLaneIdxType = typeof inLaneIdx ;
        siglane['targetLane'] = currIdx ;
        if (inLaneIdxType === 'number') {
            siglane['targetLane'] = (inLaneIdx > -1) ? inLaneIdx : currIdx ;
        } else if (inLaneIdxType !== 'undefined') {
            const inLaneIdxComponents = inLaneIdx.split(/(\+|-|\*|\/|\(|\))/) ;
            let inLaneIdxToEval = '';
            inLaneIdxComponents.map( (elem) => {
                const vldSignalId = elem.match(/^([a-z]|[A-Z]|_)([a-z]|[A-Z]|[0-9]|_)*$/);
                if (vldSignalId) {
                    const actIdx = source.config.wdScratchpad['id2LaneIdx'][elem] ;
                    if (actIdx !== undefined) {
                        inLaneIdxToEval += (actIdx.toString());
                    } else {
                        inLaneIdxToEval += '0';
                    }
                } else {
                    inLaneIdxToEval += elem ;
                }
            } );
            const possibleTgtLane = safeEvalMathExpression(inLaneIdxToEval);
            siglane['targetLane'] = ( (typeof possibleTgtLane === 'undefined') ? currIdx : possibleTgtLane ) ;
        }
    });
